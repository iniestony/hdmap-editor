...
export default class RoadLaneLineItemLaneBoundaryTypeEditTransaction extends StandardTransaction {
  ...
  reformatNewRoad() {
    ...
    if (this.laneLineSide === LaneLineSide.Inner) {
      ...
      this.scope.emitEvent(InvokeRoadLaneLineOuterAttributeEditEvent, {
        landId: this.landId,
      });
    }
  ...
  updateInnerLine(type: string) {
    ...
    if (laneItem.laneSide === LaneSide.Left) {
      const leftLaneIndex = roadItem.laneItems.leftLanes.findIndex((lane: LaneItem) => {
        return lane.laneId === laneItem.laneId
      });

      const newIndex = leftLaneIndex - 1;

      if (newIndex >= 0) {
        roadItem.laneItems.leftLanes[newIndex].laneLines.outerLaneLine.atlasLaneBoundaryType = laneLineItem.atlasLaneBoundaryType;
        roadItem.laneItems.leftLanes[newIndex].laneLines.outerLaneLine.atlasLaneBoundaryVirtual = laneLineItem.atlasLaneBoundaryVirtual;
      } else {
        const rightLanesFirst = roadItem.laneItems.rightLanes[0];

        if (rightLanesFirst) {
          const rightInnerLaneLine = rightLanesFirst.laneLines.innerLaneLine;

          rightInnerLaneLine.atlasLaneBoundaryType = laneLineItem.atlasLaneBoundaryType;
          rightInnerLaneLine.atlasLaneBoundaryVirtual = laneLineItem.atlasLaneBoundaryVirtual;
        }
      }
    } else if (laneItem.laneSide === LaneSide.Right) {
      const rightLaneIndex = roadItem.laneItems.rightLanes.findIndex((lane: LaneItem) => {
        return lane.laneId === laneItem.laneId
      });

      const newIndex = rightLaneIndex - 1;
      
      if (newIndex >= 0) {
        roadItem.laneItems.rightLanes[newIndex].laneLines.outerLaneLine.atlasLaneBoundaryType = laneLineItem.atlasLaneBoundaryType;
        roadItem.laneItems.rightLanes[newIndex].laneLines.outerLaneLine.atlasLaneBoundaryVirtual = laneLineItem.atlasLaneBoundaryVirtual;
      } else {
        const leftLanesFirst = roadItem.laneItems.leftLanes[0];

        if (leftLanesFirst) {
          const rightInnerLaneLine = leftLanesFirst.laneLines.innerLaneLine;
  ...
  updateOuterLine(type: string) {
    ...
    if (laneItem.laneSide === LaneSide.Left) {
      ...
      if (roadItem.laneItems.leftLanes[newIndex]) {
        roadItem.laneItems.leftLanes[newIndex].laneLines.innerLaneLine.atlasLaneBoundaryType = laneLineItem.atlasLaneBoundaryType;
        roadItem.laneItems.leftLanes[newIndex].laneLines.innerLaneLine.atlasLaneBoundaryVirtual = laneLineItem.atlasLaneBoundaryVirtual;
      }
      ...
      if (roadItem.laneItems.rightLanes[newIndex]) {
        roadItem.laneItems.rightLanes[newIndex].laneLines.innerLaneLine.atlasLaneBoundaryType = laneLineItem.atlasLaneBoundaryType;
        roadItem.laneItems.rightLanes[newIndex].laneLines.innerLaneLine.atlasLaneBoundaryVirtual = laneLineItem.atlasLaneBoundaryVirtual;
      }
    }
  }
