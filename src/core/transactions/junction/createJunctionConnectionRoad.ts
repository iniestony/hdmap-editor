...
export default class CreateJunctionConnectionRoadTransaction extends StandardTransaction {
  ...
  constructor(options: Object) {
    ...
    this.involvedInfoInJunction = {
      roadId: this.connectionRoadId,
      roadCategory: RoadCategory.ConnectionRoad,
      prevJunctionVertexCategory: this.startRoadVertexCategory === RoadVertexCategory.RoadStart ? JunctionVertexCategory.RoadStart : JunctionVertexCategory.RoadEnd,
      nextJunctionVertexCategory: this.endRoadVertexCategory === RoadVertexCategory.RoadStart ? JunctionVertexCategory.RoadStart : JunctionVertexCategory.RoadEnd,
    };
  }
  ...
  generateNewKeyInfo(newRoadItem: RoadItem) {
    ...
    if (correlatedLanesInStartRoadForLeftLanes.length === 0 || correlatedLanesInEndRoadForLeftLanes.length === 0) {
      ...
      // most outer lane only
      for (let i = newLanesCount - 1; i < newLanesCount; i++) {
    ...
    // small distance lane only, either left or right
    const leftLane = newKeyInfo.laneItems.leftLanes[0] as LaneItemKeyInfo;
    const leftOuterDistance = new Path3D(leftLane.laneLines.outerLaneLine.seriePoints).getDistanceAt(1);

    const rightLane = newKeyInfo.laneItems.rightLanes[0] as LaneItemKeyInfo;
    const rightOuterDistance = new Path3D(rightLane.laneLines.outerLaneLine.seriePoints).getDistanceAt(1);

    if (leftOuterDistance < rightOuterDistance) {
      newKeyInfo.laneItems.rightLanes = [];
      newRoadItem.generalRightLaneIndex = 1;
    } else {
      newKeyInfo.laneItems.leftLanes = [];
      newRoadItem.generalLeftLaneIndex = 1;
    }
  ...
  createRoad() {
    ...
    this.scope.attachConnectionRoadFromJunction(roadItem, this.involvedInfoInJunction);
  ...
  removeRoad() {
    ...
    this.scope.detachConnectionRoadFromJunction(createdRoadItem, this.involvedInfoInJunction);
