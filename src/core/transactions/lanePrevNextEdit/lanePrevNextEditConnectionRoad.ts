...
export default class LanePrevNextEditConnectionRoadTransaction extends StandardTransaction {
  private scope: ExtendedNamespace;

  private connectionLaneSide: LaneSide;
  private connectionLaneId: string;
  ...
  private targetConnectionLaneIndex?: number;
  ...
  constructor(options: Object) {
    ...
    this.connectionLaneId = (options as unknown as { connectionLaneId: string }).connectionLaneId;
  ...
  commit() {
    ...
    return { laneId: this.connectionLaneId };
  }

  onUndo() {
    super.onUndo();

    this.editOldRoadLanePrevNext();
  }
  ...
  resolveNecessaryInfo() {
    ...
    this.targetConnectionLaneIndex = (this.oldLaneItemsKeyInfo as LaneItemKeyInfo[]).findIndex((laneItemKeyInfo: LaneItemKeyInfo) => {
      return laneItemKeyInfo.laneId === this.connectionLaneId;
    });
  ...
  generateNewKeyInfoFromOldKeyInfo(oldLaneItemsKeyInfo: LaneItemKeyInfo[]) {
    ...
    const targetConnectionLaneIndex = this.targetConnectionLaneIndex as number;
    ...
    if (this.connectionLaneSide === LaneSide.Left) {
      newLaneItemsKeyInfo = oldLaneItemsKeyInfo.map((oldLaneItemKeyInfo: LaneItemKeyInfo, laneIdx: number) => {
        if (laneIdx !== targetConnectionLaneIndex) {
          return {
  ...
  editNewRoadLanePrevNext() {
    ...
    const beforeTargetLaneItem = this.scope.resolveLaneByLaneRoadIdAndRoadCategory(this.connectionLaneId, this.connectionLaneRoadId, this.connectionLaneRoadCategory) as LaneItem;
    ...
    const afterTargetLaneItem = this.scope.resolveLaneByLaneRoadIdAndRoadCategory(this.connectionLaneId, this.connectionLaneRoadId, this.connectionLaneRoadCategory) as LaneItem;
    ...
    this.scope.emitEvent(InvokeLanePrevNextEditConnectionRoadEvent, {
      laneId: this.connectionLaneId,
    });
  }

  editOldRoadLanePrevNext() {
