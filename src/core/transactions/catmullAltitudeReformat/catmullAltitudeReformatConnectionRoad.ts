...
export default class CatmullAltitudeReformatConnectionRoadTransaction extends StandardTransaction {
  ...
  generateNewKeyInfoFromOldKeyInfo(oldRoadItemKeyInfo: RoadItemKeyInfo) {
    ...
    const rawReflineCatmullPoints = [...oldRoadItemKeyInfo.referenceLine.catmullPoints];
    ...
    const reflineAltitudeCatmullPoints = [...this.newRefLineAltitudeCatmullPoints];
    const reflineAltitudeCatmullTangents = [...this.newRefLineAltitudeCatmullTangents];
    ...
    const reflineFirstIndex = 0;
    const reflineLastIndex = reflineSeriePoints.length - 1;
    ...
    newKeyInfo.laneItems.leftLanes = oldRoadItemKeyInfo.laneItems.leftLanes.map((oldLaneItemKeyInfo: LaneItemKeyInfo, laneIndex: number) => {
      ...
      const innerLaneLineSeriePoints_raw = (laneIndex === 0) ? (
        ...
        this.scope.generateHermiteSerieAlignedNumLineSeriePointsViaCatmullPointsAndCatmullTangentsWithControlRefineForConnectionRoad(innerLaneLineCatmullPoints_raw, innerLaneLineCatmullTangents_raw, reflineSeriePoints.length)
      );
      ...
      const outerLaneLineSeriePoints_raw = this.scope.generateHermiteSerieAlignedNumLineSeriePointsViaCatmullPointsAndCatmullTangentsWithControlRefineForConnectionRoad(outerLaneLineCatmullPoints_raw, outerLaneLineCatmullTangents_raw, reflineSeriePoints.length);
